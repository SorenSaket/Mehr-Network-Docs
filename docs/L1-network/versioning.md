...
The [KeyRotation](/docs/L5-services/mhr-id) identity claim is designed specifically for this: a node signs a statement with its old key saying "my new key is X", then the network recognizes the new key. This works even if the old algorithm is weakened — as long as the rotation happens before the old key is compromised.
...
PQC migration plan:
  ...
  Constraints:
    ...
    These sizes are challenging for LoRa (484-byte max packet on constrained transport class).
    Solutions:
...
Signaling uses trust flow weight (from the [voting](/docs/L6-applications/voting) TrustFlow algorithm), not node count. This prevents Sybil manipulation of governance — 50 fake nodes with 2 inbound trust edges get ~1.7 total signal weight, not 50.

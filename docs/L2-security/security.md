...
Security in Mehr is structural, not bolted on. Every layer of the protocol incorporates cryptographic protections. There is no trusted infrastructure — no certificate authorities, no trusted servers. DNS is used only for initial [genesis gateway discovery](/docs/L3-economics/token-economics#genesis-gateway-discovery), not for protocol operation.
...
#### Variable Packet Sizes and Traffic Analysis

:::caution[Trade-off]
[Variable-size packets](/docs/L0-physical/physical-transport#transport-classes-and-variable-packet-sizes) on Standard and Bulk transport classes reveal message length to a link-layer observer. This is an intentional trade-off: LoRa links (where passive RF observation is easiest) retain **fixed-size padding**. WiFi and Ethernet links (where the observer needs physical access to the LAN or a compromised node) use variable sizes for efficiency.

**Per-transport-class policy**:

| Transport Class | Padding | Rationale |
|----------------|---------|-----------|
| **Constrained** (LoRa, NB-IoT) | Fixed-size (484 bytes) | RF broadcast — any receiver in range can observe packet sizes |
| **Standard** (WiFi, BLE, cellular) | Variable | Encrypted WiFi/BLE — requires LAN access or node compromise to observe |
| **Bulk** (Ethernet, fiber, TCP/IP) | Variable | Wired — requires physical tap or switch compromise |

Applications with high privacy requirements can opt into **payload padding to MTU** on any transport class via `PathPolicy` flags, at the cost of bandwidth efficiency.
:::
...
Usable payload on constrained transport (465 max): 369 bytes (~79% efficiency)
Usable payload on standard transport (1,481 max): 1,385 bytes (~94% efficiency)
```
...
3. **Trust graph**: A Sybil attacker needs real social relationships to gain trust. Trusted peers [vouch economically](/docs/L3-economics/trust-neighborhoods#trust-based-credit) — they absorb the debts of nodes they trust, making trust costly to extend.
4. **Proof of service (demand-backed)**: Stochastic relay rewards use a [VRF-based lottery](/docs/L3-economics/payment-channels#how-stochastic-rewards-work) that produces exactly one verifiable outcome per (relay, packet) pair — preventing grinding. However, VRF alone does not prevent traffic fabrication between colluding nodes. The actual Sybil defense is [demand-backed minting](/docs/L3-economics/payment-channels#demand-backed-minting-eligibility): VRF wins only count for minting if the packet traversed a funded payment channel, and [revenue-capped minting](/docs/L3-economics/payment-channels#revenue-capped-minting) ensures self-dealing is always unprofitable (spending Y MHR on fake traffic yields at most 0.5Y in minting).
...
<details className="faq-item">
<summary>Do variable-size packets weaken traffic analysis resistance?</summary>

Only on Standard and Bulk transport classes (WiFi, Ethernet, fiber), where the observer already needs LAN access or a compromised node — the threat model is weaker than broadcast RF. Constrained-class links (LoRa, NB-IoT) retain fixed-size padding at 484 bytes, so RF observers see uniform frame sizes. Applications with high privacy requirements can opt into payload padding to MTU on any transport class. See [Variable Packet Sizes and Traffic Analysis](#variable-packet-sizes-and-traffic-analysis).

</details>

<details className="faq-item">
<summary>Does route probing expose the sender's identity?</summary>

No. ProbeResponse uses the transport layer's reverse-path routing — each relay records only the incoming interface, not the originator's identity. The structural anonymity property (no source address in packets) applies equally to probes. Residual risks include timing correlation (probe followed by transfer to same destination) and probe-ID correlation, but these are low-severity and mitigatable via `ONION_ROUTE` for probes.

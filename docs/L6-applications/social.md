...
The envelope is a lightweight, separate [DataObject](/docs/L5-services/mhr-store) that propagates freely across the mesh. It contains everything a reader needs to decide whether to fetch the full post:
...
The full post is a mutable [DataObject](/docs/L5-services/mhr-store) containing the actual content. Fetching it costs retrieval fees:
...
A mutable DataObject containing identity information. The `claims` list references [IdentityClaims](/docs/L5-services/mhr-id#claims) — including [ProfileField](/docs/L5-services/mhr-id#profile-fields) claims for display name, bio, avatar, and any other key-value data. Each claim has its own [visibility](/docs/L5-services/mhr-id#visibility-controls), so different viewers see different fields depending on their trust relationship with the profile owner.
...
Subscribe to content from a geographic area at any level of the [scope hierarchy](/docs/L3-economics/trust-neighborhoods#hierarchical-scopes):
...
Interest feeds are **sparse** — they connect people across geography. A Pokemon feed connects Portland, Tokyo, and Berlin. Content propagates through [interest relay nodes](/docs/L3-economics/propagation#interest-relay-nodes) that bridge geographic clusters, but only after [local validation](/docs/L3-economics/propagation#local-first-interest-propagation) — the author's local community must engage with the content (boost, retrieve, or curate) before interest relays forward it globally.
...
Envelopes propagate freely through [MHR-Pub](/docs/L5-services/mhr-pub) notifications. Readers browse without spending:
...
Every post is a [DataObject](/docs/L5-services/mhr-store) that requires a storage agreement. Posting costs money:
...
When readers pay to retrieve content, a portion flows back to the author via the [kickback mechanism](/docs/L5-services/mhr-store#revenue-sharing-kickback):
...
Popular content earns more kickback than it costs to store. It becomes **self-sustaining** and [propagates upward](/docs/L3-economics/propagation) through the scope hierarchy automatically:
...
1. **Local-first propagation** — Content doesn't spread globally on publication. Geographic content starts at neighborhood scope and promotes upward only when retrieval demand justifies it. Interest content starts in the author's local cluster and only propagates to distant clusters when [locally validated](/docs/L3-economics/propagation#local-first-interest-propagation) — at least one boost, multiple distinct retrievals, or curator inclusion. A garbage post tagged with a popular topic never leaves the author's neighborhood because nobody locally endorses it.
...
Envelope appears in both Portland geographic feeds and Pokemon interest feeds. Portland Pokemon intersection subscribers see it automatically. Note: up to 3 Topic scopes are allowed (e.g., `Topic("gaming", "pokemon") + Topic("events", "meetups")`), but only 1 Geo scope per post, with total scope data capped at 1 KB — see [scope constraints](/docs/L3-economics/trust-neighborhoods#hierarchical-scopes).
...
Privacy depends on [MHR-Pub](/docs/L5-services/mhr-pub) scope routing: an envelope with no declared scopes matches no `Scope(...)` subscriptions, so it is never forwarded beyond direct MHR-Pub gossip between trusted peers. Nodes only gossip unscoped envelopes to their trusted peer set.
...
- **Public posts** (scoped): Propagate per declared scopes and [propagation economics](/docs/L3-economics/propagation). Anyone can pay to read them.
...
- **No social graph leakage**: Following is a local [MHR-Pub](/docs/L5-services/mhr-pub) subscription. No central server has a copy of the social graph. Only the subscriber and the publisher know about the subscription.

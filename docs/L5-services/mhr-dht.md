...
- **Ring distance** (routing layer): `min(|a - b|, 2^128 - |a - b|)` over the destination hash space. Used for [greedy forwarding](/docs/L1-network/network-protocol#small-world-routing-model) to route packets toward their destination. This is the Kleinberg small-world model.
...
Objects can be scoped to a [trust neighborhood](/docs/L3-economics/trust-neighborhoods), meaning:
...
If the client's relay is in its [trust graph](/docs/L3-economics/trust-neighborhoods), single-source queries (N=1) are sufficient for all tiers. Multi-source queries are only needed for untrusted relays. A trusted relay has economic skin in the game — trust means absorbing the trusted node's debts, making dishonesty self-punishing.

...
MHR-Name provides human-readable names for the Mehr mesh. There is no global namespace and no central authority — names are **subjective**, resolved from each node's own position in the [trust graph](/docs/L3-economics/trust-neighborhoods). Two nodes may resolve the same name to different targets, and that's by design.
...
The scope portion uses the [HierarchicalScope](/docs/L3-economics/trust-neighborhoods#hierarchical-scopes) format. Geographic scopes are prefixed with `geo:`, interest scopes with `topic:`. The `/` separator maps to hierarchy segments (e.g., `geo:us/oregon/portland` is `Geo("us", "oregon", "portland")`).
...
2. **Trust distance** — scored from the resolver's trust graph. The binding registered by the more-trusted node wins.
   ...
   The trust score follows the same decay model as [transitive credit](/docs/L3-economics/trust-neighborhoods#trust-based-credit): full weight for direct peers, configurable ratio (default 10%) per hop, and a floor of 0.01 for completely untrusted nodes. Higher score wins.
...
| `scope` | variable | [HierarchicalScope wire format](/docs/L3-economics/trust-neighborhoods#wire-format) |
...
**[Trust Graph](/docs/L3-economics/trust-neighborhoods)**: The trust graph is the foundation of name resolution. Trust distance determines binding priority. The `trust_decay` product along the shortest path yields the trust score.

...
Storage is **pay-per-duration** — like rent, not like purchase. The data owner pays the storage node a recurring fee via their bilateral [payment channel](/docs/L3-economics/payment-channels).
...
Just like [relay traffic](/docs/L3-economics/trust-neighborhoods), storage between trusted peers is **free**:
...
- **Curators** who reference posts in [curated feeds](/docs/L6-applications/social#5-curated-feed) drive traffic to original authors, earning kickback on their own curation feed while generating kickback for the original authors too
...
Popular content that crosses the self-funding threshold climbs the [propagation hierarchy](/docs/L3-economics/propagation) automatically. Unpopular content stays local or expires. No algorithm decides what lives and what dies — economics does.

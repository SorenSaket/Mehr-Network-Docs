...
All execution is **pure deterministic computation**. Given the same inputs, any node running the same contract produces the same output. This is what makes [verification](/docs/L4-marketplace/verification) possible.
...
If a node can't execute a contract locally, it delegates to a capable neighbor via the [capability marketplace](/docs/L4-marketplace/overview):
...
The compute node receives plaintext input, executes, and returns the result. Verification is via [result hash](/docs/L4-marketplace/verification) or redundant execution. This is the cheapest and fastest option.

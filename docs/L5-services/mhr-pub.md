...
| **Scope** | Watch all activity matching a [hierarchical scope](/docs/L3-economics/trust-neighborhoods#hierarchical-scopes) — geographic or interest |
...
For [Social](/docs/L6-applications/social) content, MHR-Pub notifications carry the [PostEnvelope](/docs/L6-applications/social#postenvelope-free-layer) — the free preview layer — rather than the full post:
...
In all modes, the reader browses envelopes at zero cost. Fetching the full [SocialPost](/docs/L6-applications/social#socialpost-paid-layer) content is a separate, paid retrieval from the storage node. This separation means even LoRa nodes on PullHint subscriptions can browse headlines and decide what's worth fetching over a higher-bandwidth link later.

## Scope Subscriptions

Scope subscriptions are the primary mechanism for geographic feeds, interest feeds, and community content discovery. They build on [Hierarchical Scopes](/docs/L3-economics/trust-neighborhoods#hierarchical-scopes) to enable structured content routing.

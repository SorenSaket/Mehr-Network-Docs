...
Trust relationships are **asymmetric and revocable at any time**. Removing a node from `trusted_peers` immediately ends free relay for that node and downgrades any stored data from "trusted peer" to normal priority in the [garbage collection policy](/docs/L5-services/mhr-store#garbage-collection). Cost overrides are unidirectional — they apply to outbound traffic pricing from the configuring node only.
...
- **Used by services** — [MHR-Name](/docs/L5-services/mhr-name) scopes names by geographic scope, [MHR-Pub](/docs/L5-services/mhr-pub) supports `Scope(match)` subscriptions, [MHR-DHT](/docs/L5-services/mhr-dht#neighborhood-scoped-dht) uses scopes for content propagation boundaries, and the [Social](/docs/L6-applications/social) layer uses scopes for geographic and interest feeds

### Geo Scope Verification

Geo scopes can optionally be **verified** — see [MHR-ID](/docs/L5-services/mhr-id) for the full verification protocol. Verification methods vary by the kind of place:

| Place Type | Verification Method | Precision |
|-----------|-------------------|-----------|
| **Physical neighborhood** | [RadioRangeProof](/docs/L5-services/mhr-id-verification#radiorangeproof) — if you can hear a node's LoRa beacon, you're within physical range | ~1–15 km |
...
Interest scopes are **never verified** — anyone can declare interest in Pokemon. Verification matters for geo scopes that carry governance weight (see [Voting](/docs/L6-applications/voting)) — a node cannot vote on Portland issues without a verified Portland-area presence claim. Unverified geo scopes are still useful for content routing and feed subscriptions; they just don't carry voting rights.

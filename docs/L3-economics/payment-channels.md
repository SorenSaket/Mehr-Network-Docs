...
The stochastic model fits within [Tier 2 (economic)](/docs/L1-network/network-protocol#bandwidth-budget) of the gossip bandwidth budget even on the most constrained links.
...
## Reservation Payment: Progressive Escrow

During an active [bandwidth reservation](/docs/L1-network/network-protocol#bandwidth-reservation), the stochastic VRF lottery is replaced by **deterministic per-chunk payment** via progressive escrow. This eliminates VRF computation overhead for large transfers while aligning incentives throughout the transfer.

### Payment Model

```
Progressive escrow payment model:

  On reservation acceptance:
    Sender escrows 10% of estimated_bytes × actual_cost_per_byte
    into the bilateral channel with Neighbor A.
    (Each hop independently escrows 10% with its next hop.)

  During transfer:
    For every 1 MB transferred (configurable chunk_size):
      - Relay reports chunk delivery via DeliveryReceipt
      - Sender's channel balance decrements by chunk_size × cost_per_byte
      - Deterministic (no VRF lottery needed)

  On completion/release:
    - Unused escrow is refunded to sender's channel balance
    - Final settlement uses standard two-phase signing protocol

  On path failure mid-transfer:
    - Sender's escrow with Neighbor A settles for bytes actually transferred
    - Each downstream hop independently settles for bytes handled
    - Sender can renegotiate a new reservation for remaining data
```

### Why Progressive, Not Full Prepayment

Full prepayment creates a bad incentive — a relay that has already been paid has no financial motivation to prioritize the sender's traffic.

| Scenario | Full Prepayment | Progressive Escrow |
|----------|----------------|-------------------|
| Relay goes offline at 50% | Sender paid 100%, got 50% delivery. Dispute needed. | Sender paid ~55% (50% transferred + 10% escrow floor). Remaining 45% never left sender's wallet. |
| Sender cancels at 10% | Sender paid 100%, needs refund. Dispute needed. | Sender paid ~12% (10% transferred + initial escrow, minus remaining escrow refund). Clean settlement. |
| Normal completion | Works but locks full capital upfront | Same total cost, better capital efficiency |

### VRF Bypass During Reservations

During an active reservation, the VRF lottery is **skipped** for reserved traffic. Relays are compensated deterministically per-chunk:

```
Relay incentive during reservation:

  Reserved traffic:
    - No VRF computation per packet
    - Payment = chunk_size × cost_per_byte (deterministic)
    - Channel updated per-chunk (~every 1 MB) instead of per-lottery-win
    - 2% service burn still applies per chunk payment

  Non-reserved traffic (concurrent):
    - Standard VRF lottery still applies
    - Reserved bandwidth is ring-fenced from general relay traffic
```

### Overhead Comparison

For a 1 GB transfer at standard MTU (1,500 bytes):

| Metric | Without Reservation | With Reservation |
|--------|-------------------|-----------------|
| VRF operations | ~666,667 | 0 |
| Channel updates | ~6,667 (at 1% win rate) | 1,024 (at 1 MB chunks) |
| Overhead reduction | — | ~6.5× fewer channel updates, zero VRF compute |

### Escrow Floor Anti-DoS

The 10% escrow floor is **non-refundable** if no data transfers within the first 60 seconds. This prevents reservation DoS (reserving bandwidth without intending to use it). For a 1 GB reservation at 42 μMHR/byte, the escrow floor is ~4.2 MHR — a meaningful financial cost per attack.

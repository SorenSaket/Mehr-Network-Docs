...
In isolated partitions — where an attacker could control all nodes and nullify non-deterministic assignment — three defense layers [bound damage to a finite, predictable amount](economics/mhr-token#attack-isolated-partition): **genesis-anchored minting** prevents all minting without provable connectivity to genesis nodes during bootstrap (epoch < 100,000); **active-set-scaled emission** limits minting to the partition's fraction of the network (`min(active_nodes, 100) / 100` of full emission — a 3-node partition gets 3%); and a **2% service burn** on every payment creates a deflationary equilibrium where burn equals minting (e.g., a 3-node partition converges to 1.5M MHR regardless of duration). When such a partition reconnects, the [CRDT merge rules](economics/crdt-ledger#partition-safe-merge-rules) adopt the winning epoch's snapshot, recover missed settlements via proofs, and the bounded excess supply dilutes all holders equally — the emission schedule's geometric decay makes any supply shock negligible over time.
